...
bool UTMUPS::CheckCoords(bool utmp, bool northp, real x, real y,
  ...
  void UTMUPS::DecodeZone(const string& zonestr, int& zone, bool& northp)
  {
    ...
    char* q;
    long zone_long = strtol(c, &q, 10);          // <-- use long
    // Verify that the value fits into an int (the valid range is tiny)
    if (zone_long < static_cast<long>(std::numeric_limits<int>::min()) ||
        zone_long > static_cast<long>(std::numeric_limits<int>::max())) {
        throw GeographicErr("Zone number out of integer range in " + zonestr);
    }
    int zone1 = static_cast<int>(zone_long);     // safe conversion
